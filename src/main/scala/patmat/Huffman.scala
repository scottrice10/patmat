...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    val charList = chars.map((_,0))
    def incr(acc:List[(Char, Int)], pair:(Char,Int)): List[(Char, Int)] = {
      val accMap: Map[Char,Int] = acc.toMap
      val map = accMap + ((pair._1, accMap.getOrElse(pair._1, 0) + 1))
      map.toList
    }

    (Map[Char,Int]() foldLeft charList)(incr)
  }
  /**
